...
module WebScraper
  module TeamsScraper
    class FrenchLeagueTeamsScraper < ApplicationService
      OpenURI::Buffer.send :remove_const, 'StringMax' if OpenURI::Buffer.const_defined?('StringMax')
      OpenURI::Buffer.const_set 'StringMax', 0
      URL = "https://en.as.com/resultados/futbol/francia/equipos/"
      ...
      def save_team_info
        ligue_1_id = League.find_by(name: "Ligue 1").id
        @team_infos.each do |team_info|
          image_file = open(team_info[:url])
          team = ::Team.new(name: team_info[:name], league_id: ligue_1_id)
